...
const (
	// Version is the version of the ECS Secrets
	Version = "{{.Version}}"

	// GitDirty indicates the cleanliness of the git repo when this ecs-secrets was built
	GitDirty = {{.Dirty}}

	// GitShortHash is the short hash of this ecs-secrets build
	GitShortHash = "{{.Hash}}"
)
`
...
func main() {
	...
	if strings.TrimSpace(os.Getenv("ECS_SECRETS_RELEASE")) == "cleanbuild" {
		// 'clean' release; all other releases assumed dirty
		info.Dirty = gitDirty()
	}
	if os.Getenv("ECS_SECRETS_UNKNOWN_VERSION") == "" {
